from student_exceptions import *
import csv
...
class Student:
    def __init__(self, name, csv_filename):
        ...
        logging.info(f'Welcome new student: {name}')
    ...
    def add_score(self, subject, score):
        ...
        logging.info(f'Add score {score} for student: {self.name}, subject: {subject}')
    ...
    def add_test_result(self, subject, result):
        ...
        if result < 0 or result > 100:
            raise InvalidScoreError(result, score_type='result')

        self.test_results[subject].append(result)
        logging.info(f'Add result {result} for student: {self.name}, subject: {subject}')
